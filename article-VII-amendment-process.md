...
4. **Impact Assessment** — Affected roles (Article V), document types (Article II), and lifecycle stages (Article III).
...
- Staleness or non-compliance data from tooling (Article VI).
...
**§ 2.1. Affected-Role Review.** The author MUST identify all affected roles per Article V (Author, Reviewer, Owner, Human Consumer, Agent Consumer) and request review from a representative of each.
...
**§ 2.3.** Proposed amendments MUST conform to the format standards of Article IV.
...
2. At least one document owner (Article V) of the affected Article has approved.
...
**§ 3.3. Change Record.** Every ratified amendment MUST include a changelog entry with version increment, date, summary, and PR link, per Article III.
...
- **MAJOR** — Changes to foundational principles (Article I), removal of an Article, or breaking changes to existing practices.
- **MINOR** — New provisions, document types (Article II), roles (Article V), or tooling requirements (Article VI) that do not alter existing obligations.
- **PATCH** — Corrections, clarifications, and formatting fixes that preserve meaning.

**§ 4.2.** The repository MUST maintain a `CHANGELOG.md` per Article VI. Each entry records version, date, affected Articles, summary, and PR link.
...
**§ 5.2.** All Articles SHALL be reviewed for relevance at least once per calendar year, coinciding with a retrospective or audit (per Article III freshness SLOs).

**§ 5.3.** Tooling under Article VI SHOULD automate sunset detection. A stale provision is not automatically repealed; it is flagged for expedited review under Section 2.
...
- Security vulnerabilities in documentation practices or tooling (Article VI).
